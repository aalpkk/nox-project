"""
NOX Project — US Market Regime (Trend) Analiz
BIST regime mantığı, US parametreleriyle.
"""
import sys, os
# Core ve market modüllerini path'e ekle
_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
if _root not in sys.path:
    sys.path.insert(0, _root)

# US config'den market-specific parametreleri al
from markets.us.config import (
    MIN_AVG_VOLUME_USD, RVOL_THRESH,
    TREND_STOP, GRI_STOP, MR_STOP, DONUS_STOP, COMBO_STOP,
    TREND_TP, GRI_TP, DONUS_TP, COMBO_TP,
    QUAL_MIN_GRI, QUAL_MIN_TREND, RS_THRESHOLD,
)

# Core'dan ortak parametreleri al
...
def analyze_regime(ticker, df, xu_df, dbg=None, usd_df=None):
    """Rejim V3 analizi — tek hisse. Yeni terminoloji ile."""
    try:
        if dbg:
            dbg['total'] += 1
        n = len(df)
