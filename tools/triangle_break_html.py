"""triangle_break HTML report — briefing aesthetic, per-TF separate pages.

Reads `output/triangle_break_tr_<tf>.parquet` × 4 (run
`tools/triangle_break_scan_live.py` first) and emits 5 files per as-of:
    output/triangle_break_5h_<YYYY-MM-DD>.html      + _latest.html
    output/triangle_break_1d_<YYYY-MM-DD>.html      + _latest.html
    output/triangle_break_1w_<YYYY-MM-DD>.html      + _latest.html
    output/triangle_break_1mo_<YYYY-MM-DD>.html     + _latest.html
    output/triangle_break_scan_<YYYY-MM-DD>.html    + _latest.html  (index)

Usage:
    python tools/triangle_break_html.py [--asof 2026-04-29] [--top 30]
"""
...
from triangle_break.html_report import (
    OUT_DIR,
    TF_ORDER,
    _file_for,
    build_all,
...
def _resolve_asof(arg: str | None) -> pd.Timestamp:
    ...
    for fam in ("tr_1d", "tr_5h", "tr_1w", "tr_1M"):
...
def main() -> int:
    ...
    idx_dated = OUT_DIR / f"triangle_break_scan_{asof_str}.html"
    idx_latest = OUT_DIR / "triangle_break_scan_latest.html"
