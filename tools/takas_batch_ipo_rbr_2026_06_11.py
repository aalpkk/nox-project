"""
Takas Batch IPO-RBR — 9 watchlist tickers broker-distribution (AKD) + ICE/SM-tip.

IPO tavan-RBR izleme listesi (2026-06-11):
  Tamamlanmamış adaylar: EKDMR ATATR AKHAN LXGYO UCAYM ARFYE FRMPL
  Bu hafta FULL olanlar (referans — kazanan takas profili): BESTE GENKM

Tez kontrolü: base/dip döneminde kümülatif takas 1-3 kurumda mı toplanıyor
(re-akümülasyon) yoksa dağınık/dağıtım mı?

Outputs:
  output/takas_batch_2026-06-11_ipo_rbr.csv      — broker-level G/H/A/3A per ticker
  output/takas_batch_2026-06-11_ipo_rbr_ice.csv  — per-ticker per-day ICE (10 gün)
  output/takas_batch_2026-06-11_ipo_rbr.md       — readable summary

Run:
  PYTHONPATH=. python tools/takas_batch_ipo_rbr_2026_06_11.py
...
ASOF = "2026-06-11"
TICKERS = ["EKDMR", "ATATR", "AKHAN", "LXGYO", "UCAYM", "ARFYE", "FRMPL",
           "BESTE", "GENKM"]
HIST_DAYS = 10

OUT_BROKER_CSV = f"output/takas_batch_{ASOF}_ipo_rbr.csv"
OUT_ICE_CSV    = f"output/takas_batch_{ASOF}_ipo_rbr_ice.csv"
OUT_MD         = f"output/takas_batch_{ASOF}_ipo_rbr.md"
