"""HTML report for triangle_break — briefing aesthetic, per-TF separate pages.

Mirrors channel_break/html_report layout. Reads triangle_break_<fam>.parquet
× 4 from out_dir.
...
TF_TO_FAM = {tf: f"tr_{tf}" for tf in TF_ORDER}
...
def _file_for(tf: str, dated_label: str | None = None) -> str:
    ...
    if dated_label:
        return f"triangle_break_{slug}_{dated_label}.html"
    return f"triangle_break_{slug}_latest.html"
...
.tri-asc  { background: rgba(122,158,122,0.10); color: var(--nox-green); border-color: rgba(122,158,122,0.30); }
.tri-sym  { background: rgba(201,169,110,0.10); color: var(--nox-gold); border-color: rgba(201,169,110,0.32); }
.tri-desc { background: rgba(168,106,106,0.12); color: var(--nox-red);   border-color: rgba(168,106,106,0.32); }
...
.tf-chip.tr { color: var(--nox-orange); border-color: rgba(168,135,106,0.35); }
...
.subtype-chip b { color: #fff; font-weight: 700; }
"""
...
def _subtype_pill(subtype: str) -> str:
    label = SUBTYPE_LABEL.get(subtype, subtype.upper())
    klass = SUBTYPE_CLASS.get(subtype, "tri-sym")
    return f'<span class="tri-pill {klass}">{html.escape(label)}</span>'
...
def _rank_triangles(df: pd.DataFrame) -> pd.DataFrame:
    """Sort accepted triangles by tier_a, state, fit, contraction tightness, age asc."""
    ...
    df = df.sort_values(
        ["_tier_ord", "_state_ord", "_fit_ord", "_contract", "_age"],
        ascending=[True, True, True, True, True],
    ).reset_index(drop=True)
    return df.drop(columns=["_tier_ord", "_state_ord", "_fit_ord", "_contract", "_age"])
...
def _triangle_table(df: pd.DataFrame, *, top: int = 30) -> str:
    ...
    for i in range(len(df)):
        ...
        rows.append(
            ...
            f"<td>{_subtype_pill(r.get('triangle_subtype') or 'symmetric')}</td>"
            f"<td>{_tier_pill(bool(r.get('tier_a')))}</td>"
            f"<td class='num'>{_fmt_pct(r.get('channel_width_pct'))}</td>"
            f"<td class='num'>{_fmt_pct(r.get('width_contraction_ratio'))}</td>"
            f"<td class='num'>{_fmt_age(r.get('bars_to_apex'))}</td>"
    ...
    return (
        '<table class="cb">'
        '<thead><tr>'
        '<th>#</th><th>Ticker</th><th>State</th><th>Subtype</th><th>Tier</th>'
        '<th>Width</th><th>Contract</th><th>→Apex</th>'
        '<th>H/L</th><th>S↑·%/bar</th><th>S↓·%/bar</th>'
...
def _subtype_chips(df: pd.DataFrame) -> str:
    if df is None or df.empty:
        return ""
    counts = df["triangle_subtype"].value_counts().to_dict()
    if not counts:
        return ""
    pieces = []
    for k in ("ascending", "symmetric", "descending"):
        n = int(counts.get(k, 0))
        if n == 0:
            continue
        label = SUBTYPE_LABEL.get(k, k)
        pieces.append(
            f'<span class="subtype-chip">{html.escape(label)} <b>{n}</b></span>'
        )
    if not pieces:
        return ""
    return f'<div class="subtype-summary">{"".join(pieces)}</div>'
...
def _stack_card(cross_tf: dict[str, set[str]], min_count: int = 2, top_n: int = 18) -> str:
    ...
    for ticker, tfs in items[:top_n]:
        chips = "".join(
            f'<span class="tf-chip tr">tr_{html.escape(t)}</span>'
...
def _tf_nav(active_tf: str | None, tf_totals: dict[str, int]) -> str:
    items: list[str] = []
    items.append(
        f'<a href="triangle_break_scan_latest.html" class="{"" if active_tf else "active"}">'
        f'<span>Index</span></a>'
    )
    for tf in TF_ORDER:
...
<div class="briefing-container">
  ...
  <div class="nox-header" style="margin-bottom:1.2rem;">
    <div class="nox-logo">
      TRIANGLE BREAK<span class="proj"> · v0.1</span>
      <span class="mode">converging-line · close-only · long-only</span>
    </div>
    <div class="nox-meta">
      As-of: <b>{asof_label}</b><br>
      Aktif üçgen: <b>{grand_total}</b><br>
      generated {now_str}
    </div>
  </div>
  ...
  <div class="cb-footer">
    Pipeline: <code>triangle_break/</code> · close-only pivots · OLS line fits ·
    4 aile (5h/1d/1w/1M) · subtypes asc/sym/desc.<br>
    Driver: <code>tools/triangle_break_html.py</code>. Tier A = ≥3 H + ≥3 L pivot touch.
    Convergence gates: bars_to_apex ≥ 1 (henüz expire değil) ·
    width_contraction &lt; 0.85 (≥%15 sıkışma).<br>
    Veri: <code>output/extfeed_intraday_1h_3y_master.parquet</code> (3y, BIST 607 evren).<br>
    <b>Descriptive only</b> — backtest/ML çalışması yok. Expanding/ambiguous reddedilir.
  </div>
...
def build_tf_html(
  ...
  <div class="cb-banner">
    ...
    Daralan iki trendline (parallelism &gt; 0.25) + alt-tip filtre
    (asc / sym / desc; expanding/ambiguous out) + apex henüz geçilmemiş
    + ≥%15 width contraction. Long-only up-break.
    State'ler: <b>TRIGGER</b> (asof'ta üst-line kırılım, gates pass),
    <b>EXTENDED</b> (1-5 bar önce gates-pass kırılım),
    <b>PRE-BREAK</b> (close ±%0.5 üst trendline, henüz kırılmadı veya
    gates fail). Tier A = ≥3 H + ≥3 L pivot. Contract = width@asof /
    width@first_pivot (düşük → daha sıkışmış). Ticker → TV grafik.
  </div>
...
"""
    ...
    subtype_counts = (
        df["triangle_subtype"].value_counts().to_dict()
        if df is not None and not df.empty else {}
    )

    if df is not None and not df.empty:
        ta_df = df[df["tier_a"].fillna(False).astype(bool)]
        tier_a_tickers = ta_df["ticker"].astype(str).tolist()
        trigger_tickers = (
            df[df["signal_state"] == "trigger"]["ticker"].astype(str).tolist()
        )
    else:
        tier_a_tickers, trigger_tickers = [], []

    picks_json = {
        "schema_version": 2,
        "system": "triangle_break",
        "page": tf,
        "asof": asof_label,
        "tf_totals": tf_totals,
        ...
        "tier_a_tickers": tier_a_tickers,
        "trigger_tickers": trigger_tickers,
    }
    picks_marker = (
        '<script id="triangle-break-data" type="application/json">'
        + json.dumps(picks_json, ensure_ascii=False)
        + '</script>'
    )

    return _shell(
        page_title=f"NOX Triangle Break · {TF_LABELS[tf]} — {asof_label}",
...
def build_index_html(
    ...
    *,
    ...
    # Aggregate subtype counts across all TFs
    agg_subtypes: dict[str, int] = {}
    for df in per_tf.values():
        if df is None or df.empty:
            continue
        for k, v in df["triangle_subtype"].value_counts().items():
            agg_subtypes[str(k)] = agg_subtypes.get(str(k), 0) + int(v)
    subtype_chip_html = ""
    if agg_subtypes:
        pieces = []
        for k in ("ascending", "symmetric", "descending"):
            n = agg_subtypes.get(k, 0)
            if n == 0:
                continue
            label = SUBTYPE_LABEL.get(k, k)
            pieces.append(
        ...
        if pieces:
            subtype_chip_html = f'<div class="subtype-summary">{"".join(pieces)}</div>'
  ...
  <div class="cb-banner gold">
    <b>NOX Triangle Break v0.1</b> — TF başına ayrı sayfalar. Daralan
    iki trendline (asc / sym / desc) + apex henüz geçilmemiş + ≥%15
    width contraction + long-only up-break. Cross-TF stack panel ≥2
    TF'de fire eden ticker'ları topluyor.
  </div>
...
"""
    ...
    tier_a_per_tf: dict[str, list[str]] = {}
    trigger_per_tf: dict[str, list[str]] = {}
    for tf in TF_ORDER:
        df_tf = per_tf.get(tf, pd.DataFrame())
        if df_tf is None or df_tf.empty:
            tier_a_per_tf[tf] = []
            trigger_per_tf[tf] = []
            continue
        tier_a_per_tf[tf] = (
            df_tf[df_tf["tier_a"].fillna(False).astype(bool)]["ticker"].astype(str).tolist()
        )
        trigger_per_tf[tf] = (
            df_tf[df_tf["signal_state"] == "trigger"]["ticker"].astype(str).tolist()
        )
    ...
    picks_json = {
        ...
        "tf_totals": tf_totals,
        "grand_total": grand_total,
        "subtype_counts": agg_subtypes,
        "stack_count": len([1 for tfs in cross_tf.values() if len(tfs) >= 2]),
        "tier_a_per_tf": tier_a_per_tf,
        "trigger_per_tf": trigger_per_tf,
    }
    picks_marker = (
        '<script id="triangle-break-data" type="application/json">'
        + json.dumps(picks_json, ensure_ascii=False)
    ...
    return _shell(
        page_title=f"NOX Triangle Break · Index — {asof_label}",
        asof_label=asof_label,
        grand_total=grand_total,
        tf_totals=tf_totals,
